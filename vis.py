...
class Visualization():
	...
	def __init__(self,params):
		...
		print "N_display:",      self.N_display
	...
	def read_ranking(self, query):
		rankings_dir = self.rankings_dir
		query_name   = os.path.basename(query.split('_query')[0]) +'.txt'
		
		print "query:", 			 query
		print "rankings_dir:", rankings_dir
		print "query_name:",   query_name

		with open(os.path.join(rankings_dir, query_name), 'r') as f:
	...
	def get_query_im(self, query):
		...
		p2 			   = (bbx[2], bbx[3])
		color 		 = (255, 0, 0)
		cv2.rectangle(im, p1, p2, color, self.size_box)
	...
	def vis_one_query(self,query,ranking):
		...
		out_name = os.path.basename(query).split('_query')[0] + '.png'
		out_file = os.path.join(self.figures_path, out_name)
